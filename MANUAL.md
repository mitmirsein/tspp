...
아래에서 `<run>`은 이번 설교의 작업 폴더명(예: `2026-pentecost-acts2`). 사람 입력은 `input/<run>/`, 파이프라인 산출은 `output/<run>/`에 둔다.

```bash
RUN=2026-pentecost-acts2
python scripts/tspp.py init $RUN
```
...
python scripts/research_fanout.py "<본문/주제>" \
  --keywords-file input/$RUN/meditation_seed.json \
...
# 2) OA 일부 자동 입수 후, 설교자가 정예 2~3편을 골라 '권장 파일명' 그대로 내려받아 둔다:
python scripts/tspp.py fetch $RUN
#    input/resources/$RUN/   ← 목회자 자산(gitignore·로컬 처리)

# 3) 본문 추출(원문 페이지 번호 보존 = 정확한 인용)
python scripts/tspp.py ingest $RUN
...
python scripts/voice_resolve.py \
  ...
  --out input/$RUN/resolved_voice.json
```

단일 래퍼로는:

```bash
python scripts/tspp.py voice $RUN --genre psalm_lament --tier pastoral --season lent
```
...
python scripts/outline_preflight.py \
  --meditation-seed input/$RUN/meditation_seed.json \
  --resolved-voice  input/$RUN/resolved_voice.json \
  --evidence        output/$RUN/EvidencePack.json \   # 선택
  --out             output/$RUN/writing_brief.json
```

단일 래퍼는 기본으로 위 `input/$RUN` 경로를 사용한다.

```bash
python scripts/tspp.py preflight $RUN
python scripts/tspp.py outline-draft $RUN
```
...
python scripts/homiletic_audit.py \
  ...
  --resolved  input/$RUN/resolved_voice.json \
...
RUN=test01
python scripts/tspp.py init $RUN
# 1) 멘토 스킬에 본문+묵상 메모 제시 → input/$RUN/meditation_seed.json 산출·승인
# 2) 보이스 합성(개인 보이스/청중 없이 — L1보편+L2만)
python scripts/tspp.py voice $RUN --genre epistle --tier expository --season ordinary
# 3) 게이트 → 4) 개요 초안 골격 → 5) 개요 작성(스킬) → 6) 계기판
python scripts/tspp.py preflight $RUN
python scripts/tspp.py outline-draft $RUN
```
